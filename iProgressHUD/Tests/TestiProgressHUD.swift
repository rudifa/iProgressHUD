...
//  iProgressHUD
//
//  Created by Rudolf Farkas on 11.10.2024.
//
...
final class iProgressHUDTests: XCTestCase {
    ...
    func test_view_methods() {
        // Create a view
        let hostingView = UIView()

        // Create an instance of iProgressHUD
        let iprogress = iProgressHUD()

        // Attach the progress to the view
        iprogress.attachProgress(toView: hostingView)

        // Assert that hostingView has an attached iProgressHUD
        XCTAssertNotNil(hostingView.iprogressHud)

        // Assert that hostingView.iprogressHud is different than iprogress
        XCTAssertFalse(hostingView.iprogressHud === iprogress)

        // Assert that iprogress is not attached to the view
        // (because it attached a copy of itself)
        XCTAssertNil(iprogress.view)

        // Assert that progress is not being shown
        XCTAssertFalse(hostingView.isShowingProgress())

        // Show the progress
        hostingView.showProgress()

        // Test that the progress is showing
        XCTAssertTrue(hostingView.isShowingProgress())

        // Dismiss the progress
        hostingView.dismissProgress()

        // Test that the progress is not showing
        XCTAssertFalse(hostingView.isShowingProgress())
    }
